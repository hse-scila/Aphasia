...
class AphasiaDataset(Dataset):
    def __init__(self, csv_file, root_dir, target_sample_rate=16000, fft_size=512,
        ...
        self.data = []
        self.is_validation = "val_filenames.csv" in csv_file  # Check if the dataset is for validation
    ...
    def process_audio(self, file_path):
        ...
        if self.is_validation:
            return [self.create_spectrogram(audio)]  # Return the entire audio
...
def evaluate_model(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for spectrograms, labels in tqdm(dataloader, desc="Validation"):
            ...
            preds = torch.argmax(outputs, dim=1).cpu().numpy()  # Get predicted classes
...
if __name__ == "__main__":
    ...
    optimizer = optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)  # AdamW is better for regularization
    scheduler = CosineAnnealingLR(optimizer, T_max=10, eta_min=1e-6)  # Cosine scheduler
    num_epochs = 30
    f = open("/home/aysurkov/aphasia/log2.txt", "a")
    f.write("Train start! ")
    f.close()
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        acc, prec, rec = evaluate_model(model, val_dataloader, criterion, device)  # Use val_dataloader for validation
        scheduler.step()
    ...
    acc, prec, rec = evaluate_model(model, test_dataloader, criterion, device)  # Perform final evaluation on the test set
